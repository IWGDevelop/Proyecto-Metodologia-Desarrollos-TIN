  ...
  return (
    <div className="space-y-5 p-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <Link href="/requerimientos" className="mb-2 inline-flex items-center gap-1 text-xs text-slate-400 hover:text-slate-600">
        ...
        <Link href={`/requerimientos/${id}/editar`}>
                    ...
                    <li key={h.id} className="flex gap-3 py-3">
                      ...
                      <div className="min-w-0 flex-1 text-sm">
                        <div className="flex flex-wrap items-center gap-1">
                          {antCfg && (
                            <span className={cn('rounded-full px-2 py-0.5 text-xs', antCfg.bgColor, antCfg.textColor)}>
                              {antCfg.label}
                            </span>
                          )}
                          {antCfg && <span className="text-slate-400">→</span>}
                          <span className={cn('rounded-full px-2 py-0.5 text-xs', nuevCfg.bgColor, nuevCfg.textColor)}>
                            {nuevCfg.label}
                          </span>
                        </div>
                        {h.observacion && <p className="mt-0.5 text-xs text-slate-500">"{h.observacion}"</p>}
                        <p className="mt-0.5 text-xs text-slate-400">
                          {h.usuario ?? 'Sistema'} · {formatFechaRelativa(h.created_at)}
                        </p>
                      </div>
                    </li>
                  )
