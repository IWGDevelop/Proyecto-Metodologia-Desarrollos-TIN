...
export default async function RequerimientosPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="space-y-4 p-6">
      ...
      {/* Filtros — client component con hidratación de URL */}
      <Suspense>
        <FiltrosRequerimientos total={total} totalFiltrado={totalFiltrado} />
      </Suspense>

      {/* Tabla — client component con react-query */}
      <Suspense fallback={<TablaRequerimientosSkeleton />}>
        <TablaRequerimientos filtros={filtros} page={page} sort={sort} />
      </Suspense>
    </div>
  )
}
