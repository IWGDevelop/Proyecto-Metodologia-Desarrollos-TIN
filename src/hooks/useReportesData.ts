...
export function calcularRango(preset: FiltrosReportes['preset']): { desde: string; hasta: string } {
  ...
  switch (preset) {
    case 'mes':       return { desde: new Date(hoy.getFullYear(), hoy.getMonth(), 1).toISOString().split('T')[0], hasta }
    case 'trimestre': return { desde: new Date(hoy.getFullYear(), hoy.getMonth() - 2, 1).toISOString().split('T')[0], hasta }
    case 'semestre':  return { desde: new Date(hoy.getFullYear(), hoy.getMonth() - 5, 1).toISOString().split('T')[0], hasta }
    case 'año':       return { desde: new Date(hoy.getFullYear(), 0, 1).toISOString().split('T')[0], hasta }
...
export function useReportesData(filtros: FiltrosReportes, isAdmin = false) {
